...
with c22:
    ...
    with st.expander("¿Cómo sé en cuál circuito y mesa voté?"):
            c1111, c2244, c3321 = st.columns([0.1,0.8,0.1])
            with c2244:
    ...
    with c88:
        ...
        df_final2 = df_final.groupby(['agrupacion', 'tipovoto'])['votos'].sum().reset_index()
    ...
    colores_partido = {
        ...
        'FRENTE DE IZQUIERDA Y DE TRABAJADORES - UNIDAD': '#c11f1f',
        'EN BLANCO': '#c4c4c4',
                }
    ...
    fig = px.bar(df_sin_nulos_grafico, x='Porcentaje', y='agrupacion', color='agrupacion', color_discrete_map=colores_partido, orientation='h', text='Porcentaje', 
                 custom_data=[df_sin_nulos_grafico['votos']], title=f'Resultados en mesa {mesa} del circuito {circuito} perteneciente a {seccion}, {distrito} (Porcentaje de votos)')
